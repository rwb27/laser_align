...
class ScopeStage:
    ...
    def move_rel(self, vector, backl=None, override=False):
        """Move the stage by (x,y,z) micro steps.
        :param vector: The increment to move by along [x, y, z].
        :param backl: An array of the backlash along [x, y, z]. If None,
        ...
        # Check backl has correct format.
        if backl is None:
            backl = np.array([128, 128, 128])
        assert np.all(backl >= 0), "Backlash must >= 0 for all [x, y, z]."
        backl = h.verify_vector(backl)

        r = h.verify_vector(vector)

        # Generate the list of movements to make. If backl is [0, 0, 0],
        # there is nly one motion to make.
        movements = []
        if np.any(backl != np.zeros(3)):
            # Subtract an extra amount where vector is negative.
            r[r < 0] -= backl[np.where(r < 0)]
            r2 = np.zeros(3)
            r2[r < 0] = backl[np.where(r < 0)]
            movements.append(r2)
    ...
    def move_to_pos(self, final, overr=False):
        new_position = h.verify_vector(final)
        rel_mov = np.subtract(new_position, self.position)
        return self.move_rel(rel_mov, override=overr)
...
def tiled(stage, cam, microscope, n=10, steps=500, backlash=128, focus=False):
    """Take a tiled image of a sample, taking account of backlash.
    :param stage: The stage object.
    :param cam: The camera object, from the class Camera defined in camera.py.
    :param microscope: The microscope object, needed for calibration. TODO
    separate the GUI and microscope classes entirely. The GUI should
    instantiate a microscope object.
    :param n: The number of images per axis to take, i.e. generates an n x n
    images in total.
    :param steps: The number of steps to take between each image.
    :param backlash: The backlash.
    :param focus: Whether to autofocus between each tile. For the first
    image, a full auto-focus is performed, and a quick one on subsequent tiles.
    ...
    try:
        for j in range(n):
            for i in range(n):
                ...
                time.sleep(0.5)
                image = cam.get_frame()
                proc.crop_section(image, 50)
            stage.move_rel([0, steps, 0])
            direction *= -1
        stage.move_rel([-n / 2 * steps, -n / 2 * steps, 0])
    except KeyboardInterrupt:
        print "Aborted, moving back to start"
        stage.move_to_pos([0, 0, 0])


if __name__ == '__main__':
    stg = ScopeStage()
    camera = camera.Camera()
    microscope = microscope.MicroscopeGUI()
    camera.preview()

    tiled(stg, camera, microscope)
